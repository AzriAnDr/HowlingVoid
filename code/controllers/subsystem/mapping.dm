...
/datum/controller/subsystem/mapping/proc/LoadGroup(list/errorList, name, path, files, list/traits, list/default_traits, silent = FALSE, height_autosetup = TRUE)
	...
	if(!silent)
		add_startup_message("Loaded [name] in [(REALTIMEOFDAY - start_time)/10]s!") // NOVA EDIT CHANGE - ORIGINAL: INIT_ANNOUNCE("Loaded [name] in [(REALTIMEOFDAY - start_time)/10]s!")
	return parsed_maps
...
// BEGIN NOVA CORE MIGRATION: code/controllers/subsystem/mapping.dm
...
/datum/controller/subsystem/mapping/setup_ruins()
	...
	return ..()
// END NOVA CORE MIGRATION: code/controllers/subsystem/mapping.dm
