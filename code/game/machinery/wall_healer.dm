...
/obj/machinery/wall_healer/process()
	...
	var/blood_healing_now = round(min(initial(blood_healing) * 0.1, blood_healing, max(0, BLOOD_VOLUME_SAFE - current_user.get_blood_volume())), 0.1)
	...
	if(blood_healing_now)
		amount_healed += current_user.adjust_blood_volume(blood_healing_now, maximum = BLOOD_VOLUME_SAFE)
	...
	var/missed_blood_healing = blood_healing_now > 0 && current_user.get_blood_volume() >= BLOOD_VOLUME_SAFE
