...
/obj/item/pipe_dispenser/bluespace
	...
	icon = 'icons/bsrpd/bsrpd.dmi'
	icon_state = "bsrpd"
	lefthand_file = 'icons/bsrpd/bsrpd_left.dmi'
	righthand_file = 'icons/bsrpd/bsrpd_right.dmi'
	...
	/// Prevents spam clicking to avoid paying costs multiple times.
	var/in_use = FALSE
	/// Whether remote piping is enabled.
...
/obj/item/pipe_dispenser/bluespace/item_interaction(mob/living/user, obj/item/tool, list/modifiers)
	...
	if(istype(tool, /obj/item/assembly/signaler/anomaly/bluespace))
		...
		to_chat(user, span_warning("You cannot improve [src] further."))
		return ITEM_INTERACT_BLOCKING
...
/obj/item/pipe_dispenser/bluespace/ranged_interact_with_atom(atom/interacting_with, mob/living/user, list/modifiers)
	if(!remote_piping_toggle)
		return NONE

	if(current_capacity < ranged_use_cost)
		to_chat(user, span_warning("[src] lacks the charge to do that."))
		return ITEM_INTERACT_BLOCKING
