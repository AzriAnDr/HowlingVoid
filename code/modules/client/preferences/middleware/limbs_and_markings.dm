	...
	for(var/slot in GLOB.body_markings_per_limb)
		...
		for(var/marking_name in sort_list(GLOB.body_markings_per_limb[slot]))
			...
			slot_choices += list(list(
				"name"                = marking_name,
				"recommended_species" = marking.recommended_species ? jointext(assoc_to_keys(marking.recommended_species), ",") : null,
			))
...
/datum/preference_middleware/limbs_and_markings/proc/build_marking_presets()
	...
	for(var/preset_name in sort_list(assoc_to_keys(GLOB.body_marking_sets)))
		...
		presets += list(list(
			"name"                = preset_name,
			"recommended_species" = marking_set.recommended_species ? jointext(assoc_to_keys(marking_set.recommended_species), ",") : null,
		))
	return presets
...
/datum/preference_middleware/limbs_and_markings/get_ui_data(mob/user)
	...
	data["augments"]        = LAZYLISTDUPLICATE(preferences.augments)
	data["augment_styles"]  = LAZYLISTDUPLICATE(preferences.augment_limb_styles)
	data["marking_choices"] = build_marking_choices()
	data["marking_presets"] = build_marking_presets()
