/obj/vehicle/sealed/mecha/steam_helios
	...
	possible_int_damage = MECHA_INT_FIRE | MECHA_INT_CONTROL_LOST
...
/obj/vehicle/sealed/mecha/steam_helios/populate_parts()
	...
	scanmod = new /obj/item/stock_parts/scanning_module/triphasic/clock(src)
...
/datum/action/vehicle/sealed/mecha/judicial_mark/Trigger(mob/clicker, trigger_flags)
	if(!..())
		return
...
/datum/action/vehicle/sealed/mecha/steam_discharge/Trigger(mob/clicker, trigger_flags)
	if(!..())
		return
	if(!chassis || !(owner in chassis.occupants))
		return
	...
	for(var/steam_range in 0 to 5)
		for(var/turf/steam_turf in all_turfs)
			...
			for(var/mob/living/steam_target in steam_turf)
				...
				var/turf/thrownat = get_ranged_target_turf_direct(chassis, steam_target, 10, rand(-10, 10))
				steam_target.throw_at(thrownat, 8, 2, null, TRUE, force = MOVE_FORCE_OVERPOWERING, gentle = TRUE)
				steam_target.apply_damage((IS_CULTIST(steam_target) ? 30 : 20), BURN, wound_bonus = 30)
