...
/obj/effect/judicial_mark/proc/do_mark()
	playsound(src, 'sound/clock_cult/magic/ratvar_attack.ogg', 50, use_reverb = TRUE)
	sleep(1.6 SECONDS)
	...
	for(var/mob/living/marked_mob in range(1, src))
		...
		marked_mob.visible_message(
			span_warning("[marked_mob] is hit by a judicial explosion!"),
