...
/obj/structure/deployable_barricade
	icon = 'icons/barricades/barricade.dmi'
	anchored = TRUE
	density = TRUE
...
/obj/structure/deployable_barricade/update_overlays()
	...
	if(is_wired)
		if(!closed)
			. += image('icons/barricades/barricade.dmi', icon_state = "[barricade_type]_wire")
		else
			. += image('icons/barricades/barricade.dmi', icon_state = "[barricade_type]_closed_wire")
...
/obj/structure/deployable_barricade/metal/update_overlays()
	...
	switch(barricade_upgrade_type)
		if(BARRICADE_TYPE_BOMB)
			. += image('icons/barricades/barricade.dmi', icon_state = "+explosive_upgrade_[damage_state]")
		if(BARRICADE_TYPE_MELEE)
			. += image('icons/barricades/barricade.dmi', icon_state = "+brute_upgrade_[damage_state]")
		if(BARRICADE_TYPE_ACID)
			. += image('icons/barricades/barricade.dmi', icon_state = "+burn_upgrade_[damage_state]")
...
/obj/structure/deployable_barricade/metal/proc/attempt_barricade_upgrade(obj/item/stack/sheet/iron/metal_sheets, mob/user, modifiers)
	...
	var/static/list/cade_types = list(BARRICADE_TYPE_BOMB = image(icon = 'icons/barricades/barricade.dmi', icon_state = "explosive_obj"), BARRICADE_TYPE_MELEE = image(icon = 'icons/barricades/barricade.dmi', icon_state = "brute_obj"), BARRICADE_TYPE_ACID = image(icon = 'icons/barricades/barricade.dmi', icon_state = "burn_obj"))
		...
		for(var/direction in GLOB.cardinals)
			for(var/obj/structure/deployable_barricade/metal/plasteel/cade in get_step(src, direction))
				if(((dir & (NORTH|SOUTH) && get_dir(src, cade) & (EAST|WEST)) || (dir & (EAST|WEST) && get_dir(src, cade) & (NORTH|SOUTH))) && dir == cade.dir && cade.linked && cade.closed == closed)
					. += image('icons/barricades/barricade.dmi', icon_state = "[barricade_type]_[closed ? "closed" : "open"]_connection_[get_dir(src, cade)]")
