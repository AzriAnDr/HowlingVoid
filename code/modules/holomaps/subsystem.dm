#define IS_HOLOMAP_ROCK(tile) (istype(tile, /turf/closed/mineral) && tile.density)
#define IS_HOLOMAP_OBSTACLE(tile) (istype(tile, /turf/closed) || (locate(/obj/structure/window) in tile))
#define IS_HOLOMAP_SOFT_OBSTACLE(tile) ((locate(/obj/structure/grille) in tile) || (locate(/obj/structure/lattice) in tile))
#define IS_HOLOMAP_PATH(tile) istype(tile, /turf/open/floor)
#define HOLOMAP_Z_TRANSITION(tile) ((locate(/obj/structure/ladder) in tile) || (locate(/obj/structure/stairs) in tile))
...
/datum/controller/subsystem/holomaps/proc/generate_holomaps()
	. = TRUE
	valid_map_indexes.Cut()
	holomaps.Cut()
	extra_holomaps.Cut()
	holomap_z_transitions.Cut()
	holomap_position_to_name.Cut()
	GLOB.holomap_default_legend.Cut()
...
/datum/controller/subsystem/holomaps/proc/generate_default_holomap_legend()
	for(var/department_color in GLOB.holomap_color_to_name)
		...
		marker_color_overlay.DrawBox(department_color, 1, 1, 8, 8)
...
#undef IS_HOLOMAP_ROCK
#undef IS_HOLOMAP_OBSTACLE
#undef IS_HOLOMAP_SOFT_OBSTACLE
#undef IS_HOLOMAP_PATH
#undef HOLOMAP_Z_TRANSITION
