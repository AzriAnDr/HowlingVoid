/datum/station_holomap
	...
	/// Set to TRUE when the z-level does not have an icon formatted for station holomaps.
	var/bogus = TRUE
...
/datum/station_holomap/proc/initialize_holomap(turf/location, current_z_level, mob/user = null, reinit_base_map = FALSE, extra_overlays = list())
	bogus = FALSE
	location_turf = location
...
/datum/station_holomap/proc/generate_legend(list/overlays_to_use = list())
	...
	overlay_data.Cut()
	for(var/overlay_name as anything in overlays_to_use)
...
/// Updates the map with the provided overlays, removing old overlays first.
/datum/station_holomap/proc/update_map(list/overlays_to_use = list())
	...
	for(var/overlay in overlays_to_use)
		...
		for(var/image/map_layer as anything in overlays_to_use[overlay]["markers"])
