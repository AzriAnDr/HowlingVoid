...
/obj/machinery/station_map
	...
	icon = 'code/modules/holomaps/icons/stationmap.dmi'
	...
	/// The mob currently watching this map.
	var/mob/watching_mob
	/// The small in-world station map overlay.
	var/image/small_station_map
	/// Floor decal overlay shown under the wall map.
	var/image/floor_markings
	/// Z-level this holomap displays.
	var/current_z_level
	/// Runtime map image data for this machine.
...
/obj/machinery/station_map/proc/setup_holomap()
	...
	floor_markings = image('code/modules/holomaps/icons/stationmap.dmi', "decal_station_map")
	floor_markings.dir = dir

	update_icon()

/obj/machinery/station_map/attack_hand(mob/user, list/modifiers)
...
/obj/machinery/station_map/proc/open_map(mob/user)
	...
	if(!holomap_datum)
		message_admins("\[HOLOMAP] WARNING: Holomap at [x], [y], [z] [ADMIN_FLW(src)] had to set itself up on interact.")
		setup_holomap()

	holomap_datum.update_map(handle_overlays())

	var/datum/hud/user_hud = user.hud_used
	holomap_datum.base_map.loc = user_hud.holomap
	holomap_datum.base_map.alpha = 0

	RegisterSignal(user, COMSIG_MOB_KEYDOWN, PROC_REF(keydown))
	RegisterSignal(user, COMSIG_MOVABLE_MOVED, PROC_REF(check_position))

	playsound(src, 'code/modules/holomaps/sounds/holomap_open.ogg', 125)
...
/obj/machinery/station_map/attack_ai(mob/user)
	attack_hand(user)
...
/obj/machinery/station_map/proc/close_map()
	...
	playsound(src, 'code/modules/holomaps/sounds/holomap_close.ogg', 125)
	icon_state = initial(icon_state)
	if(watching_mob?.client && holomap_datum)
		animate(holomap_datum.base_map, alpha = 0, time = 0.5 SECONDS, easing = LINEAR_EASING)
		addtimer(CALLBACK(src, PROC_REF(finish_close_map), watching_mob), 0.5 SECONDS, TIMER_CLIENT_TIME)
	else
		finish_close_map(watching_mob)

	use_power = IDLE_POWER_USE
	holomap_datum?.reset_map()

/obj/machinery/station_map/proc/finish_close_map(mob/closed_mob)
	if(!closed_mob)
		return
	closed_mob.client?.screen -= closed_mob.hud_used.holomap
	if(holomap_datum)
		closed_mob.client?.images -= holomap_datum.base_map
	closed_mob.hud_used.holomap.used_station_map = null
	closed_mob.hud_used.holomap.used_base_map = null
	if(watching_mob == closed_mob)
		watching_mob = null
...
/obj/machinery/station_map/multitool_act(mob/living/user, obj/item/tool)
	if(!panel_open)
		to_chat(user, span_warning("You need to open the panel to change [src]'s settings!"))
		return FALSE
	...
	to_chat(user, span_info("You set [src]'s database index to [current_z_level]."))
	return TRUE
...
/obj/machinery/station_map/engineering/attack_hand(mob/user, list/modifiers)
	. = ..()
	if(.)
		holomap_datum.update_map(handle_overlays())
...
/obj/machinery/station_map/engineering/handle_overlays()
	...
	for(var/obj/machinery/firealarm/alarm as anything in GLOB.station_fire_alarms["[current_z_level]"])
		if(alarm?.z == current_z_level && alarm?.my_area?.fire)
			var/image/alarm_icon = image('code/modules/holomaps/icons/8x8.dmi', icon_state = "fire_marker")
	...
	if(length(fire_alarms))
		extra_overlays["Fire Alarms"] = list("icon" = image('code/modules/holomaps/icons/8x8.dmi', icon_state = "fire_marker"), "markers" = fire_alarms)
...
/obj/item/circuitboard/machine/station_map/screwdriver_act(mob/living/user, obj/item/tool)
	...
	if(isnull(choice) || isnull(display_map_names_paths[choice]))
		return
...
/obj/item/circuitboard/machine/station_map/engineering
	...
	desc = "A virtual map of the surrounding station. Also shows any active fire alarms."
...
/obj/machinery/station_map/strategic
	...
	icon = 'code/modules/holomaps/icons/strategic_stationmap.dmi'
