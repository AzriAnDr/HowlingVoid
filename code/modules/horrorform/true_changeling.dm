		...
		if(M && M.client)
			...
			if(M_turf && M_turf.z == src.z)
				...
				if(dist <= 7) //source of sound very close
					M.playsound_local(src, 'sound/horrorform/horror_scream.ogg', 80, 1, frequency)
				else
					var/vol = clamp(100-((dist-7)*5), 10, 100) //Every tile decreases sound volume by 5
					M.playsound_local(src, 'sound/horrorform/horror_scream_reverb.ogg', vol, 1, frequency)
...
/obj/projectile/bullet/pellet/bone_fragment
	name = "bone fragment"
	icon = 'icons/horrorform/bone_fragment.dmi'
...
/datum/action/innate/turn_to_human
	...
	button_icon = 'icons/horrorform/actions_changeling.dmi'
	button_icon = 'icons/horrorform/actions_changeling.dmi'
...
/datum/action/innate/devour/Trigger(trigger_flags)
	...
	playsound(lunch, 'sound/horrorform/tear.ogg', 50, 1)
	lunch.emote("scream")
