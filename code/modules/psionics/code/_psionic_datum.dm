...
/datum/psionic
	...
	// ХУДы
	// Сигнал
	var/atom/movable/screen/psionic/psionic_signal/signal
	// Энергия
	var/atom/movable/screen/psionic/psionic_energy/energy
...
/datum/psionic/proc/on_hud_created(datum/source)
	...
	signal = new /atom/movable/screen/psionic/psionic_signal(null, psi_hud)
	energy = new /atom/movable/screen/psionic/psionic_energy(null, psi_hud)

	psi_hud.infodisplay += signal
	psi_hud.infodisplay += energy
...
/datum/psionic/proc/remove_hud_element(mob/living/current_mob)
	var/datum/hud/hud_used = current_mob?.hud_used
	if(hud_used)
		hud_used.infodisplay -= signal
		hud_used.infodisplay -= energy
	var/client/current_client = current_mob?.client
	if(current_client)
		current_client?.screen -= signal
		current_client?.screen -= energy
...
/datum/psionic/proc/research_spell(datum/action/cooldown/spell/spell_path)
	...
	if(psi_point < initial(spell_path.point_cost))
		...
		return FALSE
	psi_owner.playsound_local(psi_owner.loc, 'code/modules/psionics/sounds/power_evoke.ogg', 50, TRUE)
...
/datum/status_effect/psionic_exhaustion/on_apply()
	. = ..()
	instabilityicon = icon('code/modules/psionics/icons/spells.dmi', "instability")
