...
/mob/living/carbon/human/proc/update_underwear(sync_inventory = TRUE)
	...
	if(bra && bra != "Nude" && !underwear_render_hidden(UNDERWEAR_HIDE_BRA))
		...
		if(bra_accessory)
			...
			bra_overlay = mutable_appearance(bra_accessory.icon, icon_state, -NOVA_BRA_LAYER)
	...
	if(socks && socks != "Nude" && num_legs >= 2 && !underwear_render_hidden(UNDERWEAR_HIDE_SOCKS))
		...
		if(isnull(taur_body) || taur_body.name == SPRITE_ACCESSORY_NONE)
			...
			if(undie_accessory)
				...
				if((bodyshape & BODYSHAPE_DIGITIGRADE))
					icon_state += "_d"
				socks_overlay = mutable_appearance(undie_accessory.icon, icon_state, -NOVA_SOCKS_LAYER)
				if(!undie_accessory.use_static)
...
// BEGIN NOVA CORE MIGRATION: code/modules/mob/living/carbon/human/human_update_icons.dm
...
/obj/effect/abstract/held_tk_effect
	name = "held_tk_effect"
	icon = 'icons/effects/tele_effects.dmi'
	icon_state = "holder"
	layer = HANDS_LAYER
...
/obj/effect/abstract/held_tk_effect/left
	is_right = FALSE
// END NOVA CORE MIGRATION: code/modules/mob/living/carbon/human/human_update_icons.dm
