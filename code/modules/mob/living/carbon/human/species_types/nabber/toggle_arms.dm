/obj/item/melee/nabber_blade
	...
	icon = 'icons/obj/species/nabber/items.dmi'
	icon_state = "mantis_arm_r"
	item_flags = ABSTRACT | DROPDEL
	w_class = WEIGHT_CLASS_HUGE
	force = 25
	throwforce = 0 // Keep detached blades harmless if they ever end up thrown.
	throw_range = 0
	throw_speed = 0
...
/datum/action/cooldown/toggle_arms
	...
	button_icon = 'icons/mob/species/nabber/actions.dmi'

	var/obj/item/restraints/handcuffs/stored_handcuffs = null // Stored cuffs restored after leaving scythe mode.
...
/datum/action/cooldown/toggle_arms/proc/rise_arms()
	...
	playsound(nabber, 'sound/mobs/humanoids/nabber/nabberscream.ogg', 70)

	if(!do_after(nabber, 2 SECONDS, nabber, IGNORE_USER_LOC_CHANGE))
		StartCooldown()
		return FALSE

	// Stores worn cuffs inside the nabber body while scythe mode is active.
	if(nabber.handcuffed)
...
/datum/action/cooldown/toggle_arms/proc/down_arms(force = FALSE)
	...
	if(force) // BYOND standard deletion guard.
		if(stored_handcuffs)
			stored_handcuffs.forceMove(stored_handcuffs.drop_location())
			stored_handcuffs = null
	...
	// Restores cuffs that were worn before entering scythe mode.
	if(stored_handcuffs)
