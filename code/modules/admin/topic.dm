...
/datum/admins/Topic(href, href_list)
	...
	if(href_list["editrightsbrowserhousekeep"])
		...
		minor_announce(
			"The emergency shuttle will reach its destination in [DisplayTimeText(timer SECONDS)].",
			message_ru = "Аварийный шаттл достигнет пункта назначения через [DisplayTimeText(timer SECONDS)].",
		)
