...
/datum/round_event_control/proc/preRunEvent()
	...
	//message_admins("Random Event triggering in [DisplayTimeText(RANDOM_EVENT_ADMIN_INTERVENTION_TIME)]: [name]. (<a href='byond://?src=[REF(src)];cancel=1'>CANCEL</a>) (<a href='byond://?src=[REF(src)];different_event=1'>SOMETHING ELSE</a>)") // NOVA EDIT REMOVAL
	//sleep(RANDOM_EVENT_ADMIN_INTERVENTION_TIME) // NOVA EDIT REMOVAL
...
// BEGIN NOVA CORE MIGRATION: code/modules/events/_event.dm
...
/datum/round_event_control/run_event(random = FALSE, announce_chance_override = null, admin_forced = FALSE, event_cause)
	. = ..()
	var/storyteller_forced = ("[event_cause]" == "the storyteller")
	log_game("ICES: [src.name] is our next event.")
	if(intensity_restriction && GLOB.intense_event_credits)
		GLOB.intense_event_credits--
		if(!storyteller_forced)
...
// END NOVA CORE MIGRATION: code/modules/events/_event.dm
