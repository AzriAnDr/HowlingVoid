...
/obj/machinery/computer/trainstation_control/Initialize(mapload, obj/item/circuitboard/C)
	...
	if(SStrain_controller?.loaded_station && !station)
		set_station(SStrain_controller.loaded_station)
...
/obj/machinery/computer/trainstation_control/ui_data(mob/user)
	...
	data["requires_password"] = station?.required_password || FALSE
	data["station_name"] = station?.name || "Unlinked Station"
...
/obj/machinery/computer/trainstation_control/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	...
	if(unlocked)
		return TRUE
	if(!station)
		balloon_alert(ui.user, "station link missing")
		return TRUE
...
/obj/effect/turf_decal/train_sigh
	name = "Train sigh"
	icon = 'icons/event/zvents/structures/train_sigh.dmi'
	icon_state = "sigh"
...
/obj/structure/prop/big/military_nuke
	...
	icon = 'icons/event/zvents/structures/props/goon/64x48.dmi'
	anchored = FALSE
	opacity = FALSE
	density = TRUE
