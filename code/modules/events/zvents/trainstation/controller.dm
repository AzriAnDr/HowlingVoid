...
/datum/controller/subsystem/train_controller/proc/show_station_logo(datum/train_station/station, silent = FALSE)
	for(var/mob/player in GLOB.player_list)
		if(!silent)
			SEND_SOUND(player, 'sound/event/zvents/effects/station_logo.ogg')
...
/datum/controller/subsystem/train_controller/proc/start_moving(force = FALSE, unload_station = TRUE)
	...
	soundloop.start()
	sound_to_playing_players('sound/machines/train/steam_short.ogg', volume = 60)
	tain_starting = FALSE
...
/datum/controller/subsystem/train_controller/proc/stop_moving()
	...
	sound_to_playing_players('sound/machines/train/steam_long.ogg', volume = 60)
	if(no_engine_mode)
...
/obj/effect/mapping_helpers/ztrait_injector/trainstation
	traits_to_add = list(ZTRAIT_NOPARALLAX = TRUE, ZTRAIT_NOXRAY = TRUE, ZTRAIT_NOPHASE = TRUE, ZTRAIT_TRAINSTATION = TRUE, ZTRAIT_BASETURF = /turf/open/space)
...
/datum/looping_sound/global_sound/train_sound_loop
	sounds_to_play = list(
		'sound/machines/train/loop_trainride.ogg' = 63 SECONDS,
	)
