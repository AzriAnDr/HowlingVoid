/obj/effect/overlay/gunpoint_effect
	icon = 'icons/gunpoint/targeted.dmi'
	icon_state = "locking"
	layer = FLY_LAYER
	plane = GAME_PLANE
...
/datum/gunpoint/proc/lock_on()
	if(src) //if we're not present then locking on failed and this datum is deleted
		...
		playsound(get_turf(source), 'sound/gunpoint/targeton.ogg', 50,1)
...
/datum/gunpoint/Destroy()
	...
	UnregisterSignal(target, list(COMSIG_QDELETING, COMSIG_ITEM_ATTACK_SELF, COMSIG_LIVING_UNARMED_ATTACK, COMSIG_ITEM_ATTACK_SELF, COMSIG_MOVABLE_RADIO_TALK_INTO, COMSIG_MOB_FIRED_GUN, COMSIG_MOVABLE_MOVED))
...
/datum/gunpoint/proc/click_destroy()
	SIGNAL_HANDLER
	if(locked)
		playsound(get_turf(source), 'sound/gunpoint/targetoff.ogg', 50,1)
	qdel(src)
