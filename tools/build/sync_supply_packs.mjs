...
function loadDefineMap() {
  ...
  const evaluateExpression = (expression) => {
    ...
    transformed = transformed.replace(/\b([A-Z][A-Z0-9_]+)\b/g, (full, token) => {
      ...
      return JSON.stringify(evaluate(token));
    });
...
function parseTopLevelBlocks(fileContents) {
  ...
  const typeHeaderPattern = /^\/datum\/supply_pack(?:\/[^\s(]*)?\s*$/;
  const procHeaderPattern = /^\/datum\/supply_pack\/[^\s]*\s*\(.*$/;
...
function rewriteSupplyPackFile(filePath) {
  ...
  for (const block of blocks) {
    ...
    if (hasChildren || hasBehavior || block.path === '/datum/supply_pack') {
      keptBlocks.push(block);
      continue;
    }
    ...
    if (!properties.has('contains') && !properties.has('cost') && !properties.has('name')) {
      keptBlocks.push(block);
      continue;
    }
...
function main() {
  ...
  for (const filePath of dmFiles) {
    const result = rewriteSupplyPackFile(filePath);
  ...
  console.log(
    `Synced supply pack manifest: exported ${exportedEntries} leaf entries from ${touchedFiles} files to ${outputPath}`,
  );
}
