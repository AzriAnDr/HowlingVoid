...
- Macro `NEUROWARE_METABOLIZE_HELPER` overrides `on_mob_end_metabolize()` for the given reagent type.
  - Used for all neuroware reagents in `former_nova_module_tree/modules/neuroware/code/datums/reagents`
...
- Bitflags for chip labels:
  ...
  - `NEUROWARE_WARD`
  - `NEUROWARE_ZENGHU`
- Defined in `code/modules/neuroware/objects/items/_neuroware.dm` are macros for manufacturer tags. Matches up to bitflags defined in `neuroware_defines.dm`:
  - `CHIP_LABEL_BISHOP`
  - `CHIP_LABEL_DEFOREST`
  - `CHIP_LABEL_DONK`
  - `CHIP_LABEL_MAINT`
  ...
  - `CHIP_LABEL_ZENGHU`
  - `NEURO_SLOT_NAME`
- Defined in `code/_globalvars/lists/reagents.dm`:
...
- `code/datums/greyscale/json_configs/gags/items/neuroware.json`
